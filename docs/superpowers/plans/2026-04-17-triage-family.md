# Triage Family Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rename architect-planner and product-planner into a cohesive triage-* family, update their label language to support cross-skill ticket refinement, and add a new triage-bugs skill with adversarial 4-pass investigation methodology.

**Architecture:** Three sibling skills sharing an identical orchestrator shell (ticket detection, disk caching, 4-agent parallel dispatch, cross-cluster notes, post-processing, cleanup). They differ in cluster definitions, sub-agent persona, investigation methodology, filing format, and ticket labels. Tickets are not owned by any skill; labels are descriptive tags that accumulate across skill runs.

**Tech Stack:** Markdown skill files (SKILL.md), git, JSON (marketplace.json)

**Spec:** `docs/superpowers/specs/2026-04-17-triage-family-design.md`

---

### Task 1: Rename architect-planner to triage-architecture

**Files:**
- Rename: `skills/architect-planner/` → `skills/triage-architecture/`
- Modify: `skills/triage-architecture/SKILL.md` (all internal references)

- [ ] **Step 1: Rename the directory with git mv**

```bash
git mv skills/architect-planner skills/triage-architecture
```

- [ ] **Step 2: Update frontmatter name**

In `skills/triage-architecture/SKILL.md`, find:
```
...
- [ ] **Step 4: Update usage examples in Mode section**
...
- [ ] **Step 5: Update cache directory name**

Find:
```
`<temp>/architect-planner-<PROJECT_ID>`.
```
Replace with:
```
...
- [ ] **Step 6: Update closed tickets filter to include bug label**

Find:
```
**Closed tickets (titles only):** Fetch recently closed tickets labeled `architecture` or `product` (titles, IDs, and labels only).
```
Replace with:
```
**Closed tickets (titles only):** Fetch recently closed tickets labeled `architecture`, `product`, or `bug` (titles, IDs, and labels only).
```

- [ ] **Step 7: Update coverage status header**
...
- [ ] **Step 8: Update state file read key**

Find:
```
Read the `architect-planner` value from the state file for the "Last run" timestamp.
```
Replace with:
```
Read the `triage-architecture` value from the state file for the "Last run" timestamp (fall back to legacy key `architect-planner` if the new key is absent).
```

- [ ] **Step 9: Update sub-agent template label language**
...
- [ ] **Step 10: Update sub-agent template cross-reference guidance**
...
- [ ] **Step 11: Update post-processor persona**

Find:
```
You are a post-processor for the architect-planner.
```
Replace with:
```
You are a post-processor for triage-architecture.
```

- [ ] **Step 12: Update state file write key**

Find:
```
set `architect-planner` to the current ISO timestamp
```
Replace with:
```
set `triage-architecture` to the current ISO timestamp
```

- [ ] **Step 13: Update state file preserve line**

Find:
```
Preserve any existing `product-planner` data.
```
Replace with:
```
Preserve any existing data for other triage skills.
```

- [ ] **Step 14: Verify and commit**

```bash
# Verify the old name doesn't appear anywhere except the legacy fallback
grep -r "architect-planner" skills/triage-architecture/ | grep -v "legacy key"

# Should return nothing (or only the state file fallback mention)
git add skills/triage-architecture/
git commit -m "feat(skill)!: rename architect-planner to triage-architecture

Update all internal references: frontmatter, cache dir, state key,
coverage header, usage examples, post-processor persona. Update label
language to support cross-skill ticket refinement (labels accumulate,
not replace). Add bug label to closed tickets filter for cross-skill
dedup."
```

---

### Task 2: Rename product-planner to triage-product

**Files:**
- Rename: `skills/product-planner/` → `skills/triage-product/`
- Modify: `skills/triage-product/SKILL.md` (all internal references)

- [ ] **Step 1: Rename the directory with git mv**

```bash
git mv skills/product-planner skills/triage-product
```

- [ ] **Step 2: Update frontmatter name**

In `skills/triage-product/SKILL.md`, find:
```
...
Find:
```
**Closed tickets (titles only):** Fetch recently closed tickets labeled `product` or `architecture` (titles, IDs, and labels only).
```
Replace with:
```
**Closed tickets (titles only):** Fetch recently closed tickets labeled `architecture`, `product`, or `bug` (titles, IDs, and labels only).
...
Find:
```
Read the `product-planner` value from the state file for the "Last run" timestamp.
```
Replace with:
```
Read the `triage-product` value from the state file for the "Last run" timestamp (fall back to legacy key `product-planner` if the new key is absent).
```
...
Find:
```
You are a post-processor for the product-planner.
```
Replace with:
```
You are a post-processor for triage-product.
```
...
Find:
```
set `product-planner` to the current ISO timestamp
```
Replace with:
```
set `triage-product` to the current ISO timestamp
```
...
Find:
```
Preserve any existing `architect-planner` data.
```
Replace with:
```
Preserve any existing data for other triage skills.
...
```bash
# Verify the old name doesn't appear anywhere except the legacy fallback
grep -r "product-planner" skills/triage-product/ | grep -v "legacy key"

# Should return nothing
git add skills/triage-product/
git commit -m "feat(skill)!: rename product-planner to triage-product
...
### Task 3: Create triage-bugs skill
...
```bash
mkdir -p skills/triage-bugs
```

- [ ] **Step 2: Write the complete SKILL.md**

Create `skills/triage-bugs/SKILL.md` with the following content:
...
- [ ] **Step 3: Verify the file structure matches siblings**

```bash
# Check frontmatter is parseable (first --- to second ---)
head -5 skills/triage-bugs/SKILL.md

# Check file size is in the expected range (siblings are 11-25 KB)
wc -c skills/triage-bugs/SKILL.md

# Check all cluster slugs appear consistently
grep -c "data-state\|security-auth\|correctness\|silent-failures" skills/triage-bugs/SKILL.md
```

Expected: frontmatter has name/description/argument-hint, file is 15-30 KB, each slug appears multiple times.

- [ ] **Step 4: Commit**

```bash
git add skills/triage-bugs/
git commit -m "feat(skill): add triage-bugs skill

Adversarial bug investigation with 4-pass method (frame, trace, falsify,
prove). Four parallel clusters: Data & State, Security & Auth, Correctness,
Silent Failures. Includes certainty bar hard gate, rejection ledger, and
...
- [ ] **Step 1: Replace architect-planner section**
...
- [ ] **Step 2: Replace product-planner section**
...
- [ ] **Step 3: Add triage-bugs section after triage-product**

Insert the following immediately after the triage-product section (before the `### [get-it-right]` section):
...
- [ ] **Step 4: Update ticket systems paragraph**

Find:
```markdown
Ticket-aware skills (`next-ticket`, `architect-planner`, `product-planner`) auto-detect your system
```

Replace with:
```markdown
Ticket-aware skills (`next-ticket`, `triage-architecture`, `triage-product`, `triage-bugs`) auto-detect your system
```
...
```bash
git add README.md
git commit -m "docs: update README for triage family rename and new triage-bugs skill"
```

---

### Task 5: Update agentic-marketplace

**Files:**
- Modify: `/Users/adam/_opensource/agentic-marketplace/.claude-plugin/marketplace.json`

- [ ] **Step 1: Update plugin description**

In `/Users/adam/_opensource/agentic-marketplace/.claude-plugin/marketplace.json`, find:
```json
"description": "Skills for ticket triage, auditing, code review, and branch shipping"
```
...
```bash
cd /Users/adam/_opensource/agentic-marketplace
git add .claude-plugin/marketplace.json
git commit -m "docs: update plugin description for triage family"
```
...
- [ ] **Step 1: Verify no stale references to old names**

```bash
cd /Users/adam/_opensource/agentic-toolkit
grep -r "architect-planner" skills/ README.md | grep -v "legacy key"
grep -r "product-planner" skills/ README.md | grep -v "legacy key"
```

Expected: no results (or only the state file fallback mentions).

- [ ] **Step 2: Verify all three triage skills exist with correct frontmatter**

```bash
head -4 skills/triage-architecture/SKILL.md
head -4 skills/triage-product/SKILL.md
head -4 skills/triage-bugs/SKILL.md
```

Expected: each shows `name: triage-architecture`, `name: triage-product`, `name: triage-bugs` respectively.
...
Expected: `code-review/`, `convert-worktree/`, `get-it-right/`, `next-ticket/`, `pr/`, `ship/`, `triage-architecture/`, `triage-bugs/`, `triage-product/`. No `architect-planner/` or `product-planner/`.

- [ ] **Step 4: Verify marketplace change**

```bash
cat /Users/adam/_opensource/agentic-marketplace/.claude-plugin/marketplace.json | grep description
```

Expected: `"description": "Skills for ticket triage (bugs, architecture, product), code review, and branch shipping"`
